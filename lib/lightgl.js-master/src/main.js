...
var GL = {
  ...
  create: function(options) {
    ...
    //That Only In Case If User Did Not Specified WebGLRenderingContext2D Options
    options = options || { antialias: false };  
    var canvas = document.createElement('canvas');
    //Canvas Made To Be Compatible With Window Size For More Better Content
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    if (!('alpha' in options)) options.alpha = false;
    try { gl = canvas.getContext('webgl', options); } catch (e) {}
    try { gl = gl || canvas.getContext('webgl2', options); } catch (e) {}
...
function addEventListeners() {
  ...
  function mousedown(e) {
    ...
    if (!isDragging()) {
      ...
      on(document,'touchmove',mousemove);
      on(document,'touchcancel',mouseup);
      off(document,'touchmove',mousemove);
      off(gl.canvas, 'mousemove', mousemove);
      off(gl.canvas, 'mouseup', mouseup);
      off(gl.canvas,'touchmove',mousemove);
      off(gl.canvas,'touchcancel',mouseup);
      off(document,'touchmove',mousemove);
      off(document,'touchcancel',mouseup);
    }
